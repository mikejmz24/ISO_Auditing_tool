...
--             json_object(
--                 'level_name', (
--                     SELECT requirement_level.level_name FROM requirement
--                     INNER JOIN requirement_level ON requirement.level_id = requirement_level.id
--                 )
...
, requirements_data AS (
    SELECT
        ...
        , json_arrayagg(
            json_object(
                'level_name', rl.level_name  -- Direct reference to joined table column
                , 'reference_code', r.reference_code
                , 'name', r.name
                , 'description', r.description
                , 'questions', (
    ...
    FROM requirement AS r
    INNER JOIN requirement_level AS rl ON r.level_id = rl.id  -- Join here
    GROUP BY r.standard_id
)
