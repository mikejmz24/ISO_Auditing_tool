...
type DraftRepositoryTestSuite struct {
	suite.Suite
	mockRepo *mocks.MockDraftRepository
	repo     repositories.DraftRepository
}

func (s *DraftRepositoryTestSuite) SetupTest() {
	s.mockRepo = new(mocks.MockDraftRepository)
	// Use real repository with mock DB if testing actual implementation
	// Or use mock repository directly for unit tests
}

func (s *DraftRepositoryTestSuite) TestCreateDraft() {
	ctx := context.Background()
	draft := testutils.CreateTestDraft()

	s.mockRepo.On("CreateDraft", ctx, draft).Return(draft, nil)

	result, err := s.mockRepo.CreateDraft(ctx, draft)
...
func TestDraftRepository(t *testing.T) {
	suite.Run(t, new(DraftRepositoryTestSuite))
}
